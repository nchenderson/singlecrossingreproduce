...
ff <- function(x) {
  ans <- Spwexp(x, rate=r15, intervals=int5) -  Spwexp(x, rate=r05, intervals=int5)
  return(ans)
}
cross.point <- uniroot(ff, interval=c(.1, 7.9))$root

tau <- 7  ## may want to change this
t1 <- 2
t2 <- 4
rmst0.true <- integrate(Spwexp, lower=0, upper=tau, rate=r05, intervals=int5, subdivisions=1000L)$value
rmst1.true <- integrate(Spwexp, lower=0, upper=tau, rate=r15, intervals=int5, subdivisions=1000L)$value
sfdiff.true <- Spwexp(t1, rate=r15, intervals=int5) - Spwexp(t1, rate=r05, intervals=int5) 
ssdiff.true <- Spwexp(t2, rate=r15, intervals=int5) - Spwexp(t2, rate=r05, intervals=int5)
survcross.true <- Spwexp(cross.point, rate=r15, intervals=int5)
rmstdiff.true <- rmst1.true - rmst0.true
num1 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r15, intervals=int5, subdivisions=1000L)$value  
num0 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r05, intervals=int5, subdivisions=1000L)$value  
...
for(k in 1:nreps) {
  times0.true <- rpwexp(n, rate=r05, intervals=int5)
  times1.true <- rpwexp(n, rate=r15, intervals=int5)
...
save(theta.hat, RMSTDiff, SFDiff, SSDiff, RRMLDiff, Stheta, RMSTDiff.KM, SFDiff.KM, SSDiff.KM,
     ...
     file="~/Documents/DelayedTreatment/SimulationResults/EstimationRuns/run5_n800.RData")
