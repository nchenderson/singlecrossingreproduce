...
ff <- function(x) {
  ans <- Spwexp(x, rate=r16, intervals=int6) -  Spwexp(x, rate=r06, intervals=int6)
  return(ans)
}
cross.point <- uniroot(ff, interval=c(2, 6))$root

tau <- 7  
t1 <- 2
t2 <- 4
rmst0.true <- integrate(Spwexp, lower=0, upper=tau, rate=r06, intervals=int6, subdivisions=1000L)$value
rmst1.true <- integrate(Spwexp, lower=0, upper=tau, rate=r16, intervals=int6, subdivisions=1000L)$value
sfdiff.true <- Spwexp(t1, rate=r16, intervals=int6) - Spwexp(t1, rate=r06, intervals=int6) 
ssdiff.true <- Spwexp(t2, rate=r16, intervals=int6) - Spwexp(t2, rate=r06, intervals=int6)
survcross.true <- Spwexp(cross.point, rate=r16, intervals=int6)
rmstdiff.true <- rmst1.true - rmst0.true
num1 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r16, intervals=int6, subdivisions=1000L)$value  
num0 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r06, intervals=int6, subdivisions=1000L)$value  
...
set.seed(57216)
theta.hat <- min.obs <- gamma.hat <- RMST0 <- RMST1 <- RMSTDiff <- rep(0, nreps)
SFDiff <- SSDiff <- Stheta <- RRML0 <- RRML1 <- RRMLDiff <- rep(0, nreps)
RMST.KM0 <- RMST.KM1 <- RMSTDiff.KM <- SFDiff.KM <- SSDiff.KM <- rep(0, nreps)
for(k in 1:nreps) {
  times0.true <- rpwexp(n, rate=r06, intervals=int6)
  times1.true <- rpwexp(n, rate=r16, intervals=int6)
...
Results6 <- Results
ResultsKM6 <- ResultsKM

save(theta.hat, RMSTDiff, SFDiff, SSDiff, RRMLDiff, Stheta, RMSTDiff.KM, SFDiff.KM, SSDiff.KM,
     rmstdiff.true, sfdiff.true, ssdiff.true, rrmldiff.true, Results6, ResultsKM6, 
     file="~/Documents/DelayedTreatment/SimulationResults/EstimationRuns/run6_n800.RData")
