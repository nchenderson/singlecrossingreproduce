...
ff <- function(x) {
  ans <- Spwexp(x, rate=r13, intervals=int3) -  Spwexp(x, rate=r03, intervals=int3)
  return(ans)
}
...
t1 <- 2
t2 <- 4
rmst0.true <- integrate(Spwexp, lower=0, upper=tau, rate=r03, intervals=int3, subdivisions=1000L)$value
rmst1.true <- integrate(Spwexp, lower=0, upper=tau, rate=r13, intervals=int3, subdivisions=1000L)$value
sfdiff.true <- Spwexp(t1, rate=r13, intervals=int3) - Spwexp(t1, rate=r03, intervals=int3) 
ssdiff.true <- Spwexp(t2, rate=r13, intervals=int3) - Spwexp(t2, rate=r03, intervals=int3)
survcross.true <- Spwexp(cross.point, rate=r13, intervals=int3)
rmstdiff.true <- rmst1.true - rmst0.true
num1 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r13, intervals=int3, subdivisions=1000L)$value  
num0 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r03, intervals=int3, subdivisions=1000L)$value  
...
for(k in 1:nreps) {
  times0.true <- rpwexp(n, rate=r03, intervals=int3)
  times1.true <- rpwexp(n, rate=r13, intervals=int3)
...
Results3 <- Results
ResultsKM3 <- ResultsKM

save(theta.hat, RMSTDiff, SFDiff, SSDiff, RRMLDiff, Stheta, RMSTDiff.KM, SFDiff.KM, SSDiff.KM,
     rmstdiff.true, sfdiff.true, ssdiff.true, rrmldiff.true, Results3, ResultsKM3, 
     file="~/Documents/DelayedTreatment/SimulationResults/EstimationRuns/run3_n800.RData")
