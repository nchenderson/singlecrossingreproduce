...
t1 <- 2
t2 <- 4
rmst0.true <- integrate(Spwexp, lower=0, upper=tau, rate=r01, intervals=int1, subdivisions=1000L)$value
rmst1.true <- integrate(Spwexp, lower=0, upper=tau, rate=r11, intervals=int1, subdivisions=1000L)$value
sfdiff.true <- Spwexp(t1, rate=r11, intervals=int1) - Spwexp(t1, rate=r01, intervals=int1) 
ssdiff.true <- Spwexp(t2, rate=r11, intervals=int1) - Spwexp(t2, rate=r01, intervals=int1)
survcross.true <- 1
rmstdiff.true <- rmst1.true - rmst0.true
rrmldiff.true <- rmstdiff.true
...
Results[1,] <- c(mean(theta.hat), median(theta.hat), sd(theta.hat), mean(theta.hat^2), 0)
...
Results[6,] <- c(mean(Stheta), median(Stheta), sd(Stheta), mean((Stheta - 1)^2), 1)
...
Results1 <- Results
ResultsKM1 <- ResultsKM

save(theta.hat, RMSTDiff, SFDiff, SSDiff, RRMLDiff, Stheta, RMSTDiff.KM, SFDiff.KM, SSDiff.KM,
     rmstdiff.true, sfdiff.true, ssdiff.true, rrmldiff.true, Results1, ResultsKM1, 
     file="~/Documents/DelayedTreatment/SimulationResults/EstimationRuns/run1_n800.RData")
