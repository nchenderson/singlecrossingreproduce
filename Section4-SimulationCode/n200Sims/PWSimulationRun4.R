...
ff <- function(x) {
  ans <- Spwexp(x, rate=r14, intervals=int4) -  Spwexp(x, rate=r04, intervals=int4)
  return(ans)
}
...
t1 <- 2
t2 <- 4
rmst0.true <- integrate(Spwexp, lower=0, upper=tau, rate=r04, intervals=int4, subdivisions=1000L)$value
rmst1.true <- integrate(Spwexp, lower=0, upper=tau, rate=r14, intervals=int4, subdivisions=1000L)$value
sfdiff.true <- Spwexp(t1, rate=r14, intervals=int4) - Spwexp(t1, rate=r04, intervals=int4) 
ssdiff.true <- Spwexp(t2, rate=r14, intervals=int4) - Spwexp(t2, rate=r04, intervals=int4)
survcross.true <- Spwexp(cross.point, rate=r14, intervals=int4)
rmstdiff.true <- rmst1.true - rmst0.true
num1 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r14, intervals=int4, subdivisions=1000L)$value  
num0 <- integrate(Spwexp, lower=cross.point, upper=tau, rate=r04, intervals=int4, subdivisions=1000L)$value  
...
for(k in 1:nreps) {
  times0.true <- rpwexp(n, rate=r04, intervals=int4)
  times1.true <- rpwexp(n, rate=r14, intervals=int4)
...
Results4 <- Results
ResultsKM4 <- ResultsKM

save(theta.hat, RMSTDiff, SFDiff, SSDiff, RRMLDiff, Stheta, RMSTDiff.KM, SFDiff.KM, SSDiff.KM,
     rmstdiff.true, sfdiff.true, ssdiff.true, rrmldiff.true, Results4, ResultsKM4, 
     file="~/Documents/DelayedTreatment/SimulationResults/EstimationRuns/run4_n200.RData")
